...
router.get("/getAllUsers", async(req,res)=>{
    try {
        const result = await db.query("SELECT users.id, users.name, users.email , restaurant.user_id AS rest_user_id FROM users INNER JOIN restaurant ON restaurant.user_id != users.id GROUP BY restaurant.user_id")
        console.log(result.rows)
        res.status(200).json({
            data: result.rows
        })
