...
router.put("/update/:id", upload.single('image'), async (req, res) => {
    ...
    try {
        let result
        if (!image) {
            result = await db.query("UPDATE menu_restaurant SET page=$1, returning *", [req.body.page])
        }
        result = await db.query("UPDATE menu_restaurant SET page=$1, menu_url=$2, returning *", [req.body.page, image])
